...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav navbar-right">
      <form action="stakeHolder_logout" method="post">
...
<div class="main">
	<div id="profile" >
		<i><h2 style="margin-left:370px;">Stake Holder Profile</h2></i>
			...
			if (session != null) {
				if (session.getAttribute("emailId") != null) {
					...
					//"select * from userid WHERE id = '" + id + "' 
				      //AND pwd = '" + pwd + "'";
					String dev_sql ="SELECT * FROM STAKEHOLDER where STAKE_EMAILID = ?";
					...
					while(resultSet_dev.next()){
						dev_id=resultSet_dev.getInt("STAKE_ID");
		...
		<div class="row">
    		...
    		<div class="col-sm-6">
    			<div class="container">
	    			<i><b>NAME : <%=resultSet_dev.getString("STAKE_NAME") %></b></i>
				    <br><br>
				    <i><b>EMAIL ID : <%=resultSet_dev.getString("STAKE_EMAILID") %></b></i>
				    <br><br>
				    <i><b>COMPANY NAME : <%=resultSet_dev.getString("STAKE_CNAME") %></b></i>
    			</div>
		 	</div>
		</div>
	</div>
	...
	<br><br>
		<div id="projects">
		<% String proj_sql = "SELECT * from PROJECT where STAKE_EMAILID = ?"; 
		...
		%>
			...
			<% 
					...
					response.sendRedirect("stakeHolder_login.jsp");
				}
			}
		} catch (Exception e) {
		e.printStackTrace();
